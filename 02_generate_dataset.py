    ...
    def branchexeclp(self, allowaddcons):
        ...
        if query_expert:
            ...
            state_khalil = utilities.extract_khalil_variable_features(
                self.model, cands, self.khalil_root_buffer
            )
            ...
            if not any([s < 0 for s in scores]):
                ...
                filename = (
                    f"{self.out_dir}/sample_{self.episode}_{self.sample_counter}.pkl"
                )
        ...
        if not query_expert or (
            not self.follow_expert and self.exploration_policy != "vanillafullstrong"
        ):
        ...
        if (
            query_expert
            and self.follow_expert
            or self.exploration_policy == "vanillafullstrong"
        ):
            assert result == scip.SCIP_RESULT.DIDNOTRUN
...
def make_samples(in_queue, out_queue):
    ...
    while True:
        ...
        print(
            f"[w {os.getpid()}] episode {episode}, seed {seed}, processing instance '{instance}'..."
        )
        ...
        print(
            f"[w {os.getpid()}] episode {episode} done, {branchrule.sample_counter} samples"
        )
...
def collect_samples(
    ...
    for i in range(n_jobs):
        p = mp.Process(
            target=make_samples, args=(orders_queue, answers_queue), daemon=True
        )
        workers.append(p)
        p.start()
        ...
        while current_episode in buffer and buffer[current_episode]:
            ...
            for sample in samples_to_write:
                ...
                # else write sample
                    ...
                    i += 1
                    print(
                        f"[m {os.getpid()}] {i} / {n_samples} samples written, ep {sample['episode']} ({in_buffer} in buffer)."
                    )
