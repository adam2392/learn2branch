...
class PreNormLayer(K.layers.Layer):
    ...
    def __init__(self, n_units, shift=True, scale=True):
        ...
        if shift:
            self.shift = self.add_weight(
                ...
                initializer=tf.keras.initializers.constant(
                    value=np.zeros((n_units,)), dtype=tf.float32
                ),
            )
        else:
            self.shift = None
        ...
        if scale:
            self.scale = self.add_weight(
                ...
                trainable=False,
                initializer=tf.keras.initializers.constant(
                    value=np.ones((n_units,)), dtype=tf.float32
                ),
            )
        else:
            self.scale = None
    ...
    def update_stats(self, input):
        ...
        """
        assert (
            self.n_units == 1 or input.shape[-1] == self.n_units
        ), f"Expected input dimension of size {self.n_units}, got {input.shape[-1]}."
    ...
    def stop_updates(self):
        ...
        if self.scale is not None:
            self.var = tf.where(
                tf.equal(self.var, 0), tf.ones_like(self.var), self.var
            )  # NaN check trick
...
class BipartiteGraphConvolution(K.Model):
    ...
    def build(self, input_shapes):
        ...
        self.output_module.build(
            [None, self.emb_size + (l_shape[1] if self.right_to_left else r_shape[1])]
        )
        self.built = True
    ...
    def call(self, inputs, training):
        ...
        joint_features = self.feature_module_final(
            tf.gather(
                self.feature_module_left(left_features), axis=0, indices=edge_indices[0]
            )
    ...
    def pre_train_next_rec(model, name):
        for layer in model.layers:
            if isinstance(layer, K.Model):
                ...
                isinstance(layer, PreNormLayer)
                and layer.waiting_updates
                and layer.received_updates
            ):
                layer.stop_updates()
...
class GCNPolicy(BaseModel):
    ...
    def __init__(self):
        ...
        self.conv_v_to_c = BipartiteGraphConvolution(
            self.emb_size, self.activation, self.initializer, right_to_left=True
        )
        ...
        self.input_signature = [
            (
                tf.contrib.eager.TensorSpec(
                    shape=[None, self.cons_nfeats], dtype=tf.float32
                ),
                tf.contrib.eager.TensorSpec(shape=[2, None], dtype=tf.int32),
                tf.contrib.eager.TensorSpec(
                    shape=[None, self.edge_nfeats], dtype=tf.float32
                ),
                tf.contrib.eager.TensorSpec(
                    shape=[None, self.var_nfeats], dtype=tf.float32
                ),
