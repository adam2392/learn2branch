...
def init_scip_params(
    model, seed, heuristics=True, presolving=True, separating=True, conflict=True
):
...
def extract_state(model, buffer=None):
    ...
    if "state" in buffer:
        ...
        coef_matrix = sp.vstack(
            (-coef_matrix[has_lhs, :], coef_matrix[has_rhs, :])
        ).tocoo(copy=False)
    ...
    u, s_, vt = svds(adj, k=3)
    x_hat = u @ np.diag(np.sqrt(s_))
    ...
    col_feats["sol_frac"][
        s["col"]["types"] == 3
    ] = 0  # continuous have no fractionality
...
def extract_khalil_variable_features(model, candidates, root_buffer):
    ...
    variable_features = np.stack(
        [scip_state[feature_name] for feature_name in variable_feature_names], axis=1
    )
